...
screen/                      # اسکرین‌شات‌های محیط اجرا و تاییدیه تصویری
verification/                # اسکریپت‌های تست Playwright
```

---

## ۷. فرآیند تایید و تست (Verification)
برای اطمینان از عملکرد صحیح برنامه و جلوگیری از بروز ارور در هنگام بیلد:
- **تست تایپ (TypeScript):** با اجرای دستور `npx tsc --noEmit` تمامی فایل‌ها از نظر صحت تایپ بررسی شده‌اند.
- **تست رابط کاربری (UI Testing):** با استفاده از Playwright، اسکرین‌شات‌هایی از تمامی بخش‌های برنامه تهیه شده که در پوشه `screen/` موجود است.
- **تست خودکار:** اسکریپت `verification/test_app.py` وظیفه پیمایش خودکار در برنامه و ثبت وضعیت بصری را بر عهده دارد.
